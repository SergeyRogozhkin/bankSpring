...
public class CustomerControllerTestNew {
    ...
    void updateCustomer() throws Exception {
        ...
        Mockito.when(customerService.updateCustomer(Mockito.anyLong(), Mockito.any(CustomerEntity.class)))
                .thenReturn(customerMapper.toDto(customerUpdate));
    ...
    void deleteCustomer() throws Exception {
        CustomerEntity customerDeleted = new CustomerEntity();
        customerDeleted.setName("Customer Deleted");
        customerDeleted.setArchive(true);

        Mockito.when(customerService.deleteCustomer(Mockito.anyLong()))
                .thenReturn(customerMapper.toDto(customerDeleted));

        mvc.perform(MockMvcRequestBuilders.delete("/customer/{id}", Mockito.anyLong()))
                .andExpect(status().isOk())
                .andDo(result -> System.out.println(result.getResponse().getContentAsString()))
                .andExpect(content().json("{\"archive\": true}"));
    }
